...
const arrowStyle: JSX.CSSProperties = {
    ...
    return (
        ...
        <span><span class="head">AC-Head: </span><span>{(() => { const h= props.meta.fHeading.toFixed(1); return `${h}`; })()}</span></span>
        ...
        <span><span class="head">Dist: </span><span>{(() => { const h= props.distBear.dist.toFixed(1); return `${h}`; })()} </span><span class="head"> Bear: </span><span>{(() => { const v= props.distBear.bear.toFixed(1); return `${v}`; })()}</span></span>   
        </div>
        </div>
